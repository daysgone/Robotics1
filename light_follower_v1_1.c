    ...
    while(1) 
    {
        ...
        else
        {
            ...
            if( abs(light_sensor_dif) > sensor_threshold)
            {
				...
				if( light_sensor_dif < (0 - sensor_threshold) )
                {
					if(light_sensor_dif < -100)
                   {
					   printf("turn right hard %d", light_sensor_dif);
						mav(0, r_vel + (light_sensor_dif * 10) ) ;
						mav(1, l_vel);
					}
					if(light_sensor_dif < -35)
					{
